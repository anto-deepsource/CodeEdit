...
final class CEWorkspaceFile: Codable, Comparable, Hashable, Identifiable, TabBarItemRepresentable {
  ...
  func labelFileName() -> String {
    ...
    case .showOnly:
      return self.fileName(
        typeHidden: !prefs.shownFileExtensions.extensions.contains(self.type.rawValue))
    case .hideOnly:
      return self.fileName(
        typeHidden: prefs.hiddenFileExtensions.extensions.contains(self.type.rawValue))
    }
  }
  ...
  func validateFileName(for newName: String) -> Bool {
    ...
    guard
      !newName.isEmpty && newName.isValidFilename
  ...
  func addFolder(folderName: String) {
    ...
    while CEWorkspaceFile.fileManger.fileExists(atPath: folderUrl.path) {
      fileNumber += 1
      folderUrl = folderUrl.deletingLastPathComponent().appendingPathComponent(
  ...
  var nearestFolder: URL {
    (self.isFolder ? self.url : self.url.deletingLastPathComponent())
  }
  ...
  func duplicate() {
    ...
    while CEWorkspaceFile.fileManger.fileExists(atPath: fileUrl.path) {
      ...
      let fileName =
        ...
        ? previousName : previousName.replacingOccurrences(of: ".\(fileExtension)", with: "")
      fileUrl = fileUrl.deletingLastPathComponent().appendingPathComponent(
